...
def _get_path_extensions():
    return list(
        OrderedDict.fromkeys(["", *os.environ.get("PATHEXT", "").lower().split(os.pathsep)])
    )
...
class PythonInfo:  # noqa: PLR0904
    ...
    def __init__(self) -> None:  # noqa: PLR0915
        ...
        # intentionally select `posix_prefix` which is the unaltered posix-like paths
            ...
            self.sysconfig_paths = {
                i: sysconfig.get_path(i, expand=False) for i in sysconfig.get_path_names()
            }
            self.distutils_install = self._distutils_install().copy()

        # https://bugs.python.org/issue22199
        makefile = getattr(
            sysconfig, "get_makefile_filename", getattr(sysconfig, "_get_makefile_filename", None)
        )
        self.sysconfig = {
            k: v
            for k, v in [
    ...
    def _fast_get_system_executable(self):
        ...
        if not (
            self.real_prefix or (self.base_prefix is not None and self.base_prefix != self.prefix)
        ):
    ...
    def from_exe(  # noqa: PLR0913
        ...
        proposed = from_exe(
            cls, app_data, exe, env=env, raise_on_error=raise_on_error, ignore_cache=ignore_cache
        )
