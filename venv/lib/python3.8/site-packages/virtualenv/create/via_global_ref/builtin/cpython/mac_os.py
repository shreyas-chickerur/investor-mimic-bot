...
class CPythonmacOsFramework(CPython, ABC):
    ...
    def create(self):
        ...
        for src in self._sources:
            if isinstance(src, ExePathRefToDest) and (
                src.must == RefMust.COPY or not self.symlinks
            ):
...
def fix_mach_o(exe, current, new, max_size):
    ...
    try:
        ...
        LOGGER.warning(
            "Could not call _builtin_change_mac_o: %s. Trying to call install_name_tool instead.", e
        )
        try:
...
def _builtin_change_mach_o(maxint):  # noqa: C901
    ...
    class FileView:
        ...
        def _checkwindow(self, seek_to, op):
            if not (self._start <= seek_to <= self._end):
                msg = (
                    f"{op} to offset {seek_to:d} is outside window [{self._start:d}, {self._end:d}]"
                )
                raise OSError(msg)
    ...
    def mach_o_change(at_path, what, value):  # noqa: C901
        ...
        def do_macho(file, bits, endian):
            ...
            _cpu_type, _cpu_sub_type, _file_type, n_commands, _size_of_commands, _flags = read_data(
                file, endian, 6
            )
