...
def periodic_update(  # noqa: PLR0913
    ...
    def _update_wheel(ver):
        ...
        LOGGER.debug(
            "using %supdated wheel %s", "periodically " if updated_wheel else "", updated_wheel
        )
...
def handle_auto_update(
    ...
    if u_log.needs_update:
        ...
        trigger_update(
            distribution, for_py_version, wheel, search_dirs, app_data, periodic=True, env=env
        )
...
def manual_upgrade(app_data, env):
    ...
    for for_py_version, distribution_to_package in BUNDLE_SUPPORT.items():
        ...
        for distribution in distribution_to_package:
            thread = Thread(
                target=_run_manual_upgrade, args=(app_data, distribution, for_py_version, env)
            )
            thread.start()
