...
def get_wheel(  # noqa: PLR0913
    ...
    if not download or version != Version.bundle:
        ...
        wheel = from_bundle(
            distribution, version, for_py_version, search_dirs, app_data, do_periodic_update, env
        )
...
def download_wheel(
    ...
    process = Popen(
        cmd, env=env, stdout=PIPE, stderr=PIPE, universal_newlines=True, encoding="utf-8"
    )
...
def find_compatible_in_house(distribution, version_spec, for_py_version, in_folder):
    ...
    if version_spec is not None and version_spec:
        ...
        start = next(
            (at for at, w in enumerate(wheels) if op(w.version_tuple, version)), len(wheels)
        )
...
def pip_wheel_env_run(search_dirs, app_data, env):
    env = env.copy()
    env.update(
        {"PIP_USE_WHEEL": "1", "PIP_USER": "0", "PIP_NO_INPUT": "1", "PYTHONIOENCODING": "utf-8"}
    )
    wheel = get_wheel(
        distribution="pip",
        version=None,
