...
def _is_singleton(node):  # type: (ast.AST) -> bool
    return isinstance(node, ast.Constant) and isinstance(
        node.value, (bool, type(Ellipsis), type(None))
    )
...
class Definition(Binding):
    ...
    def redefines(self, other):
        return super().redefines(other) or (
...
class Builtin(Definition):
    ...
    def __repr__(self):
        return "<{} object {!r} at 0x{:x}>".format(self.__class__.__name__, self.name, id(self))
...
class FunctionScope(Scope):
    ...
    usesLocals = False
    alwaysUsed = {"__tracebackhide__", "__traceback_info__", "__traceback_supplement__"}
...
def _is_typing_helper(node, is_name_match_fn, scope_stack):
    ...
    def _bare_name_is_attr(name):
        for scope in reversed(scope_stack):
            if name in scope:
                return (
                    isinstance(scope[name], ImportationFrom)
                    and scope[name].module in TYPING_MODULES
                    and is_name_match_fn(scope[name].real_name)
                )
    ...
    def _module_scope_is_typing(name):
        for scope in reversed(scope_stack):
            if name in scope:
                return (
                    isinstance(scope[name], Importation) and scope[name].fullName in TYPING_MODULES
                )

        return False

    return (isinstance(node, ast.Name) and _bare_name_is_attr(node.id)) or (
...
def is_typing_overload(value, scope_stack):
    return isinstance(value.source, (ast.FunctionDef, ast.AsyncFunctionDef)) and any(
...
class Checker:
        ...
        for scope in self.deadScopes:
            ...
            if undefined:
                ...
                if scope.importStarred:
                    ...
                    for name in undefined:
                        self.report(
                            messages.ImportStarUsage, scope["__all__"].source, name, from_list
                        )
    ...
    def getCommonAncestor(self, lnode, rnode, stop):
        if stop in (lnode, rnode) or not (
            hasattr(lnode, "_pyflakes_parent") and hasattr(rnode, "_pyflakes_parent")
        ):
            return None
        if lnode is rnode:
            return lnode
    ...
    def differentForks(self, lnode, rnode):
        ...
        if parts:
            for items in parts:
                if self.descendantOf(lnode, items, ancestor) ^ self.descendantOf(
                    rnode, items, ancestor
                ):
                    return True
        return False
    ...
    def addBinding(self, node, value):
        ...
        if value.name not in self.scope or not isinstance(value, Annotation):
            ...
            while isinstance(value, NamedExprAssignment) and isinstance(
                self.scopeStack[cur_scope_pos], GeneratorScope
            ):
                cur_scope_pos -= 1
    ...
    def handleNodeStore(self, node):
        ...
        if isinstance(self.scope, FunctionScope) and name not in self.scope:
            ...
            for scope in self.scopeStack[:-1]:
                ...
                if used and used[0] is self.scope and name not in self.scope.globals:
                    ...
                    self.report(
                        messages.UndefinedLocal, scope[name].used[1], name, scope[name].source
                    )
                    break
        ...
        if isinstance(parent_stmt, ast.AnnAssign) and parent_stmt.value is None:
            ...
            parent_stmt != node._pyflakes_parent and not self.isLiteralTupleUnpacking(parent_stmt)
        ):
    ...
    # "stmt" type nodes
    DELETE = FOR = ASYNCFOR = WHILE = WITH = WITHITEM = ASYNCWITH = EXPR = ASSIGN = handleChildren

    PASS = ignore

    # "expr" type nodes
    BOOLOP = UNARYOP = SET = ATTRIBUTE = STARRED = NAMECONSTANT = NAMEDEXPR = handleChildren
    ...
    def _handle_string_dot_format(self, node):
        ...
        missing_arguments = (placeholder_positional | placeholder_named) - (
    ...
    def CALL(self, node):
        ...
        if _is_typing(node.func, "cast", self.scopeStack) and len(node.args) >= 1:
            ...
            if len(node.args) > 1 and isinstance(node.args[1], ast.Dict):
                ...
                not_annotated += [
                    (arg, ["values"] if i == 1 else None) for i, arg in enumerate(node.args)
                ]
            ...
            if (
                ...
                and all(
                    isinstance(x, (ast.Tuple, ast.List)) and len(x.elts) == 2
                    for x in node.args[1].elts
                )
            ):
                omit += ["args"]
                ...
                not_annotated += [
                    (arg, ["elts"] if i == 1 else None) for i, arg in enumerate(node.args)
                ]
    ...
    def _handle_percent_format(self, node):
        ...
        if (
            ...
            not any(isinstance(elt, ast.Starred) for elt in node.right.elts)
        ):
        ...
        if isinstance(node.right, ast.Dict) and all(
            isinstance(k, ast.Constant) and isinstance(k.value, str) for k in node.right.keys
        ):
    ...
    AND = (
        ...
        NOT
    ) = UADD = USUB = EQ = NOTEQ = LT = LTE = GT = GTE = IS = ISNOT = IN = NOTIN = MATMULT = ignore
    ...
    def COMPARE(self, node):
        ...
        for op, right in zip(node.ops, node.comparators):
            if isinstance(op, (ast.Is, ast.IsNot)) and (
                _is_const_non_singleton(left) or _is_const_non_singleton(right)
            ):
