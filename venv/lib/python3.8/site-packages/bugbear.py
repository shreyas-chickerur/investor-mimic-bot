...
class BugBearChecker:
    ...
    def gen_line_based_checks(self):
        ...
        """
        noqa_type_ignore_regex = re.compile(r"#\s*(noqa|type:\s*ignore|pragma:)[^#\r\n]*$")
    ...
    def add_options(optmanager) -> None:
        ...
        if "pep8ext_naming" not in sys.modules.keys():
            optmanager.add_option(
                ...
                help=(
                    "List of method decorators that should be treated as classmethods" " by B902"
                ),
            )
    ...
    def should_warn(self, code) -> bool:
        ...
        LOG.info(
            "Optional warning %s not present in selected warnings: %r. Not " "firing it at all.",
            code,
            self.options.select,
        )
        return False
...
class BugBearVisitor(ast.NodeVisitor):
    ...
    def visit_ExceptHandler(self, node: ast.ExceptHandler) -> None:
        ...
        if self.b040_caught_exception is not None and self.b040_caught_exception.has_note:
    ...
    def check_for_b006_and_b008(self, node) -> None:
        visitor = FunctionDefDefaultsVisitor(B006, B008, self.b008_b039_extend_immutable_calls)
    ...
    def check_for_b039(self, node: ast.Call) -> None:
        ...
        visitor = FunctionDefDefaultsVisitor(B039, B039, self.b008_b039_extend_immutable_calls)
    ...
    def check_for_b012(self, node) -> None:
        def _loop(node, bad_node_types) -> None:
            ...
            if isinstance(node, (ast.While, ast.For)):
                ...
                self.errors.append(B012(node.lineno, node.col_offset, vars=(self.in_trystar,)))
    ...
    def check_for_b013_b014_b029_b030(self, node: ast.ExceptHandler) -> list[str]:
        ...
        if len(names) == 0 and not bad_handlers and not ignored_handlers:
            self.errors.append(B029(node.lineno, node.col_offset, vars=(self.in_trystar,)))
        elif (
            len(names) == 1
    ...
    def check_for_b023(self, loop_node) -> None:  # noqa: C901
        ...
        for node in ast.walk(loop_node):
            ...
            if isinstance(node, ast.Call):
                ...
                if (
                    isinstance(node.func, ast.Name) and node.func.id in ("filter", "reduce", "map")
                ) or (
                    isinstance(node.func, ast.Attribute)
    ...
    def check_for_b904(self, node) -> None:
        ...
        if (
            ...
            self.errors.append(B904(node.lineno, node.col_offset, vars=(self.in_trystar,)))
    ...
    def check_for_b032(self, node) -> None:
        if (
            ...
            and (
                ...
                or (isinstance(node.target, ast.Attribute) and node.target.value.id != "self")
            )
        ):
    ...
    def check_for_b034(self, node: ast.Call) -> None:
        ...
        def check(num_args: int, param_name: str) -> None:
            if len(node.args) > num_args:
                ...
                self.errors.append(B034(arg.lineno, arg.col_offset, vars=(func.attr, param_name)))
    ...
    def check_for_b909(self, node: ast.For) -> None:
        ...
        for mutation in itertools.chain.from_iterable(m for m in checker.mutations.values()):
...
class NameFinder(ast.NodeVisitor):
    ...
    def visit_Name(self, node: ast.Name) -> None:  # noqa: B906 # names don't contain other names
...
class FunctionDefDefaultsVisitor(ast.NodeVisitor):
    ...
    def __init__(
        ...
        self.b008_b039_extend_immutable_calls = b008_b039_extend_immutable_calls or set()
...
B016 = Error(
    message=("B016 Cannot raise a literal. Did you intend to return it or raise " "an Exception?")
)
B017 = Error(
    message=(
...
B036 = Error(message="B036 Don't except `BaseException` unless you plan to re-raise it.")

B037 = Error(message="B037 Class `__init__` methods must not return or yield any values.")
...
B040 = Error(message="B040 Exception with added note not used. Did you forget to raise it?")
...
B907 = Error(
    message=(
        "B907 {!r} is manually surrounded by quotes, consider using the `!r` conversion" " flag."
    )
)
B908 = Error(
    message=(
        "B908 assertRaises-type context should not contain more than one top-level" " statement."
    )
)
B909 = Error(
    message=("B909 editing a loop's mutable iterable often leads to unexpected results/bugs")
)
B910 = Error(message="B910 Use Counter() instead of defaultdict(int) to avoid excessive memory use")
B911 = Error(message="B911 `itertools.batched()` without an explicit `strict=` parameter.")
