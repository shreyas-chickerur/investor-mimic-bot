...
def generate_bin_op_both_wrappers(
    ...
    emitter.emit_line(
        "if (PyObject_IsInstance(obj_left, (PyObject *){})) {{".format(emitter.type_struct_name(cl))
    )
    gen.emit_arg_processing(error=GotoHandler("typefail"), raise_exception=False)
...
def generate_richcompare_wrapper(cl: ClassIR, emitter: Emitter) -> str | None:
    ...
    emitter.emit_line(
        "static PyObject *{name}(PyObject *obj_lhs, PyObject *obj_rhs, int op) {{".format(name=name)
    )
