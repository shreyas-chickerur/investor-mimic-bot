...
from .metadata import LEGACY_METADATA_FILENAME, WHEEL_METADATA_FILENAME, Metadata
from .util import (
    Cache,
    CSVReader,
    CSVWriter,
...
class Wheel(object):
    ...
    def _get_dylib_cache(self):
        ...
        if cache is None:
            ...
            base = os.path.join(
                get_cache_base(), str("dylib-cache"), "%s.%s" % sys.version_info[:2]
            )
            cache = Cache(base)
        return cache
            ...
            for arch in arches:
                ...
                if abi != "none" and sys.platform.startswith("linux"):
                    ...
                    if len(parts) == 2:
                        if parts >= (2, 5):
                            result.append(
                                ("".join((IMP_PREFIX, version)), abi, "manylinux1_%s" % arch)
                            )
                        if parts >= (2, 12):
                            result.append(
                                ("".join((IMP_PREFIX, version)), abi, "manylinux2010_%s" % arch)
                            )
                        if parts >= (2, 17):
                            result.append(
                                ("".join((IMP_PREFIX, version)), abi, "manylinux2014_%s" % arch)
                            )
                        result.append(
                            (
