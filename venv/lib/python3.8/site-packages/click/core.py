...
from . import types
from .exceptions import Abort, BadParameter, ClickException, Exit, MissingParameter, UsageError
...
class Command(BaseCommand):
    ...
    def invoke(self, ctx: Context) -> t.Any:
        ...
        """
        if self.deprecated:
            message = _("DeprecationWarning: The command {name!r} is deprecated.").format(
                name=self.name
            )
...
class Argument(Parameter):
    ...
    def _parse_decls(
        ...
        if len(decls) == 1:
            ...
            raise TypeError(
                "Arguments take exactly one parameter declaration, got" f" {len(decls)}."
            )
        return name, [arg], []
