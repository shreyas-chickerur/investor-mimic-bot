...
from setuptools.extern import packaging, six
...
class Distribution(_Distribution):
    ...
    def __init__(self, attrs=None):
        ...
        _Distribution.__init__(
            self, {k: v for k, v in attrs.items() if k not in self._DISTUTILS_UNSUPPORTED_METADATA}
        )
    ...
    def exclude_package(self, package):
        ...
        if self.ext_modules:
            self.ext_modules = [
                p for p in self.ext_modules if p.name != package and not p.name.startswith(pfx)
            ]
    ...
    def include_in(self, dist):
        ...
        if not self.available:
            raise DistutilsPlatformError(
                self.description + " is required, " "but is not available on this platform"
            )
