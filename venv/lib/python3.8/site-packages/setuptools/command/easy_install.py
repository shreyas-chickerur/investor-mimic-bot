    ...
    def easy_install(self, spec, deps=False):
        ...
        with self._tmpdir() as tmpdir:
            ...
            dist = self.package_index.fetch_distribution(
                spec, tmpdir, self.upgrade, self.editable, not self.always_copy, self.local_index
            )
            if dist is None:
    ...
    def install_site_py(self):
        ...
        if os.path.exists(sitepy):
            ...
            if not current.startswith("def __boot():"):
                raise DistutilsError(
                    "%s is not a setuptools-generated site.py; please" " remove it." % sitepy
                )
...
class WindowsExecutableLauncherWriter(WindowsScriptWriter):
    ...
    def _get_script_args(cls, type_, name, header, script_text):
        ...
        yield (name + ".exe", get_win_launcher(launcher_type), "b")  # write in binary mode
        if not is_64bit():
