...
def parse_line_ranges(line_ranges: Sequence[str]) -> List[Tuple[int, int]]:
    ...
    for lines_str in line_ranges:
        ...
        if len(parts) != 2:
            raise ValueError(
                "Incorrect --line-ranges format, expect 'START-END', found" f" {lines_str!r}"
            )
        try:
            start = int(parts[0])
            end = int(parts[1])
            ...
            raise ValueError(
                "Incorrect --line-ranges value, expect integer ranges, found" f" {lines_str!r}"
            ) from None
        else:
...
class _TopLevelStatementsVisitor(Visitor[None]):
    ...
    def visit_suite(self, node: Node) -> Iterator[None]:
        ...
        if semantic_parent is not None and not _get_line_range(semantic_parent).intersection(
            self._lines_set
        ):
