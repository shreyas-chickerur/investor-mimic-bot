...
def transform_line(line: Line, mode: Mode, features: Collection[Feature] = ()) -> Iterator[Line]:
    ...
    line_str_hugging_power_ops = _hugging_power_ops_line_to_string(line, features, mode) or line_str
...
def _first_right_hand_split(
    ...
    if (
        ...
        while (
            ...
            and inner_body_leaves[-1].opening_bracket is inner_body_leaves[unpacking_offset]
        ):
            if unpacking_offset:
        ...
        if hugged_opening_leaves and inner_body_leaves:
            ...
            if line.mode.magic_trailing_comma and inner_body_leaves[-1].type == token.COMMA:
                ...
                line_length = line.mode.line_length - sum(
                    len(str(leaf)) for leaf in hugged_opening_leaves + hugged_closing_leaves
                )
