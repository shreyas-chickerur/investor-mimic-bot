...
class Line:
        ...
        for leaf_id, comments in self.comments.items():
            for comment in comments:
                if is_type_comment(comment):
                    if comment_seen or (
                        not is_type_ignore_comment(comment) and leaf_id not in ignored_ids
                    ):
                        return True
    ...
    def has_magic_trailing_comma(self, closing: Leaf) -> bool:
        ...
        """
        if not (
            closing.type in CLOSING_BRACKETS and self.leaves and self.leaves[-1].type == token.COMMA
        ):
            return False
...
def is_line_short_enough(line: Line, *, mode: Mode, line_str: str = "") -> bool:  # noqa: C901
