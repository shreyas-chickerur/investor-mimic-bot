...
class StringMerger(StringTransformer, CustomSplitMapMixin):
    ...
    def _remove_backslash_line_continuation_chars(
        ...
        if not indices_to_transform:
            return TErr(
                "Found no string leaves that contain backslash line continuation" " characters."
            )
...
class StringSplitter(BaseStringSplitter, CustomSplitMapMixin):
    ...
    def do_transform(self, line: Line, string_indices: List[int]) -> Iterator[TResult[Line]]:
        ...
        use_custom_breakpoints = bool(
            custom_splits and all(csplit.break_idx <= max_break_width for csplit in custom_splits)
        )
        ...
        while more_splits_should_be_made():
            if use_custom_breakpoints:
                ...
                max_bidx = (
                    count_chars_in_width(rest_value, max_break_width) - string_op_leaves_length
                )
    ...
    def _get_break_idx(self, string: str, max_break_idx: int) -> Optional[int]:
        ...
        def passes_all_checks(i: Index) -> bool:
            ...
            is_big_enough = (
                len(string[i:]) >= self.MIN_SUBSTR_SIZE and len(string[:i]) >= self.MIN_SUBSTR_SIZE
            )
            return (
...
class StringParenWrapper(BaseStringSplitter, CustomSplitMapMixin):
    ...
    def _return_match(LL: List[Leaf]) -> Optional[int]:
        ...
        if parent_type(LL[0]) in [syms.return_stmt, syms.yield_expr] and LL[0].value in [
            "return",
            "yield",
        ]:
            is_valid_index = is_valid_index_factory(LL)
