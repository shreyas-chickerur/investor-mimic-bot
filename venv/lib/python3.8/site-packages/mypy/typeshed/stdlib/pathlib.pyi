...
class PurePath(PathLike[str]):
    ...
    if sys.version_info >= (3, 12):
        ...
        def __init__(
            self, *args: StrPath
        ) -> None: ...  # pyright: ignore[reportInconsistentConstructor]
    else:
    ...
    if sys.version_info >= (3, 12):
        def relative_to(
            self, other: StrPath, /, *_deprecated: StrPath, walk_up: bool = False
        ) -> Self: ...
    else:
...
class Path(PurePath):
    if sys.version_info >= (3, 12):
        def __new__(
            cls, *args: StrPath, **kwargs: Unused
        ) -> Self: ...  # pyright: ignore[reportInconsistentConstructor]
    else:
        def __new__(cls, *args: StrPath, **kwargs: Unused) -> Self: ...
    ...
    if sys.version_info >= (3, 14):
        def copy(
            self, target: StrPath, *, follow_symlinks: bool = True, preserve_metadata: bool = False
        ) -> None: ...
        def copytree(
            self,
            target: StrPath,
    ...
    if sys.version_info >= (3, 14):
        def delete(
            self, ignore_errors: bool = False, on_error: Callable[[OSError], object] | None = None
        ) -> None: ...
    ...
    if sys.version_info < (3, 12):
        if sys.version_info >= (3, 10):
            @deprecated(
                "Deprecated as of Python 3.10 and removed in Python 3.12. Use hardlink_to() instead."
            )
