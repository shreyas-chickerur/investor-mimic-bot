...
if sys.platform == "win32":
    ...
    def PeekNamedPipe(
        handle: int, size: int = 0, /
    ) -> tuple[int, int] | tuple[bytes, int, int]: ...
    if sys.version_info >= (3, 10):
    ...
    @overload
    def ReadFile(
        handle: int, size: int, overlapped: Literal[False] = False
    ) -> tuple[bytes, int]: ...
    @overload
    ...
    def SetNamedPipeHandleState(
        named_pipe: int,
        mode: int | None,
        max_collection_count: int | None,
        collect_data_timeout: int | None,
        /,
    ) -> None: ...
    def TerminateProcess(handle: int, exit_code: int, /) -> None: ...
    def WaitForMultipleObjects(
        handle_seq: Sequence[int], wait_flag: bool, milliseconds: int = 0xFFFFFFFF, /
    ) -> int: ...
    def WaitForSingleObject(handle: int, milliseconds: int, /) -> int: ...
    def WaitNamedPipe(name: str, timeout: int, /) -> None: ...
    @overload
    def WriteFile(
        handle: int, buffer: ReadableBuffer, overlapped: Literal[True]
    ) -> tuple[Overlapped, int]: ...
    @overload
    def WriteFile(
        handle: int, buffer: ReadableBuffer, overlapped: Literal[False] = False
    ) -> tuple[int, int]: ...
    @overload
    def WriteFile(
        handle: int, buffer: ReadableBuffer, overlapped: int | bool
    ) -> tuple[Any, int]: ...
    @final
    class Overlapped:
        event: int
    ...
    if sys.version_info >= (3, 13):
        ...
        def CreateEventW(
            security_attributes: int, manual_reset: bool, initial_state: bool, name: str | None
        ) -> int: ...
    ...
    if sys.version_info >= (3, 12):
        def CopyFile2(
            ...
            new_file_name: str,
            flags: int,
            progress_routine: int | None = None,
        ) -> int: ...
