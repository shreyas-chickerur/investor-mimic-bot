...
class MessageBuilder:
    ...
    def overload_signature_incompatible_with_supertype(
        ...
        self.fail(f'Signature of "{name}" incompatible with {target}', context, code=codes.OVERRIDE)
    ...
    def yield_from_invalid_operand_type(self, expr: Type, context: Context) -> Type:
        text = (
            format_type(expr, self.options) if format_type(expr, self.options) != "object" else expr
        )
            ...
            if (
                ...
                if len(subtype.items) != len(supertype.items):
                    if supertype_label is not None and subtype_label is not None:
                        ...
                        return True
                self.generate_incompatible_tuple_error(supertype.items, subtype.items, context, msg)
                return True
        return False
