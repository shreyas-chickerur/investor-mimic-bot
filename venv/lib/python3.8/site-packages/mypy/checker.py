...
from mypy import join, message_registry, nodes, operators
...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
    ...
    def check_match_args(self, var: Var, typ: Type, context: Context) -> None:
        ...
        if not self.scope.active_class():
            return
        typ = get_proper_type(typ)
        if not isinstance(typ, TupleType) or not all(is_string_literal(item) for item in typ.items):
            self.msg.note(
    ...
    def check_metaclass_compatibility(self, typ: TypeInfo) -> None:
        ...
        metaclasses = [
            ...
            if entry.metaclass_type and not is_named_instance(entry.metaclass_type, "builtins.type")
        ]
        if not metaclasses:
            return
        ...
        if isinstance(lvalue, (TupleExpr, ListExpr)):
            ...
            if lvalue_type:
                ...
                if (
                    ...
                    and (p_lvalue_type.item.type.is_abstract or p_lvalue_type.item.type.is_protocol)
                ):
        ...
        for possible_iterable_type in possible_iterable_types:
            if item_str_literals and isinstance(possible_iterable_type, TypedDictType):
                for key in item_str_literals:
                    ...
                    elif key in possible_iterable_type.items or not possible_iterable_type.is_final:
                        if_types.append(possible_iterable_type)
    ...
    def refine_identity_comparison_expression(
        ...
        if coerce_only_in_literal_context:
            ...
            def should_coerce_inner(typ: Type) -> bool:
                typ = get_proper_type(typ)
                return is_literal_type_like(typ) or (isinstance(typ, Instance) and typ.type.is_enum)
    ...
    def find_tuple_len_narrowing(self, node: ComparisonExpr) -> list[tuple[TypeMap, TypeMap]]:
        ...
        for op, items in chained:
            ...
            if op in ("is", "=="):
                ...
                yes_type, no_type = self.narrow_with_len(self.lookup_type(left.args[0]), op, r_size)
    ...
    def is_literal_enum(self, n: Expression) -> bool:
        ...
        return (
            member_type.is_enum_literal() and member_type.fallback.type == parent_type.type_object()
        )
