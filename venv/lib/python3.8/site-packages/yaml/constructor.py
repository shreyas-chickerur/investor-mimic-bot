    ...
    def check_state_key(self, key):
        ...
        if self.get_state_keys_blacklist_regexp().match(key):
            raise ConstructorError(
                None, None, "blacklisted key '%s' in instance state found" % (key,), None
            )
...
class SafeConstructor(BaseConstructor):
    ...
    def construct_yaml_binary(self, node):
        try:
            ...
            raise ConstructorError(
                None, None, "failed to convert base64 data into ascii: %s" % exc, node.start_mark
            )
        try:
...
class FullConstructor(SafeConstructor):
    ...
    def find_python_module(self, name, mark, unsafe=False):
        ...
        if name not in sys.modules:
            raise ConstructorError(
                "while constructing a Python module", mark, "module %r is not imported" % name, mark
            )
