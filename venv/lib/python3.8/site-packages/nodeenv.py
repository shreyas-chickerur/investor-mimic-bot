...
class Config(object):
    ...
    def _dump(cls):
        ...
        print(
            "    "
            + "\n    ".join(
                "%s = %s" % (k, v) for k, v in sorted(iteritems(vars(cls))) if not k.startswith("_")
            )
        )
...
def make_parser():
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", dest="verbose", default=False, help="Verbose mode"
    )

    parser.add_argument(
        "-q", "--quiet", action="store_true", dest="quiet", default=False, help="Quiet mode"
    )
    ...
    parser.add_argument(
        "--prompt", dest="prompt", help="Provides an alternative prompt prefix for this environment"
    )
...
def install_packages(env_dir, args):
    ...
    if args.npm == "latest" or real_npm_ver >= "1.0.0":
        ...
        cmd = (
            ". " + _quote(activate_path) + " && npm install %(pack)s" + " && npm activate %(pack)s"
        )
    ...
    for name, content in files.items():
        ...
        if is_CYGWIN:
            _, cyg_bin_dir = callit(
                ["cygpath", "-w", os.path.abspath(bin_dir)], show_stdout=False, in_shell=False
            )
