...
class DataQualityChecker:
    ...
    def __init__(self, artifacts_dir="artifacts/data_quality"):
        ...
        # Note: Execution engine filters to 150 days before calling this, so require at least 60 days
        ...
        logger.info(
            f"DataQualityChecker initialized: staleness={self.staleness_threshold_hours}h, "
            f"max_nan={self.max_nan_pct:.1%}"
        )

    def check_data_quality(
        self, market_data: pd.DataFrame, asof_date: datetime
    ) -> tuple[set[str], dict]:
        """
        ...
        if market_data.empty:
            ...
            raise ValueError("Market data is empty — cannot proceed; aborting trading run")
        ...
        if blocked_symbols:
            logger.warning(
                f"Data quality check: {len(blocked_symbols)}/{len(symbols)} symbols blocked"
            )
    ...
    def _check_staleness(self, symbol_data: pd.DataFrame, asof_date: datetime) -> tuple[bool, str]:
    ...
    def _check_nan_values(self, symbol_data: pd.DataFrame) -> tuple[bool, str]:
        ...
        # Note: Execution engine filters to last 150 days before calling this
        # For 150-day filtered data, sma_200 will have ~50 NaN at start (33%)
        ...
        for indicator in self.required_indicators:
            if indicator in symbol_data.columns:
                ...
                # Relaxed threshold for long-period indicators (sma_200)
                # sma_200 needs 200 days warmup, so will have ~50 NaN in 150-day window (33%)
                threshold = (
                    0.35 if indicator in ["sma_200", "price_to_sma200"] else self.max_nan_pct
                )
    ...
    def _add_issue(self, quality_report: dict, issue_type: str, symbol: str, reason: str):
        ...
        quality_report["issues"][issue_type]["symbols"].append({"symbol": symbol, "reason": reason})
