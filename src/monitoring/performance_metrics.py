...
class PerformanceMetrics:
    ...
    def add_daily_return(
        self, date: datetime, portfolio_value: float, cash: float, positions_value: float
    ):
    ...
    def calculate_metrics(self) -> dict:
        ...
        if len(self.daily_returns) > 1:
            ...
            sharpe = (
                np.sqrt(252) * (returns_array.mean() / returns_array.std(ddof=1))
        ...
        if len(self.equity_curve) > 1:
            total_return = (
                self.equity_curve[-1]["portfolio_value"] - self.equity_curve[0]["portfolio_value"]
            ) / self.equity_curve[0]["portfolio_value"]
        ...
        if len(self.daily_returns) > 1:
            ...
            sortino = (
                np.sqrt(252) * (returns_array.mean() / downside_std) if downside_std > 0 else 0
            )
        else:
            sortino = 0
