...
class StrategyHealthScorer:
    ...
    def __init__(self, db, artifacts_dir="artifacts/health"):
        ...
        logger.info(
            f"StrategyHealthScorer initialized: windows={self.short_window}d/{self.long_window}d"
        )
    ...
    def _calculate_health_score(self, metrics: dict) -> tuple[float, str, list[str]]:
        ...
        if metrics["trade_count_30d"] < self.min_trades_for_scoring:
            if no_signals_either:
                ...
                issues.append(
                    "No activity in 30d (0 signals, 0 trades) — event-driven silence or new deployment"
                )
            else:
                score -= 30
                issues.append(
                    f"Low trade count: {metrics['trade_count_30d']} in 30d (need {self.min_trades_for_scoring})"
                )
        ...
        if metrics["rejection_rate_30d"] > self.max_rejection_rate:
            score -= 20
            issues.append(
                f"High rejection rate: {metrics['rejection_rate_30d']:.1%} (threshold: {self.max_rejection_rate:.1%})"
            )
    ...
    def generate_health_summary(self, strategies: list[tuple[int, str]]) -> str:
        ...
        avg_health_score = (
            sum(s["health_score"] for s in strategy_health) / len(strategy_health)
            if strategy_health
            else 0
        )
        ...
        summary = {
            ...
            "summary": {
                ...
                "warning": len(
                    [s for s in strategy_health if s["health_status"] in ["WARNING", "DEGRADED"]]
                ),
    ...
    def get_health_summary_for_email(self, strategies: list[tuple[int, str]]) -> str:
        ...
        for strategy_id, strategy_name in strategies:
            ...
            status_emoji = {"HEALTHY": "✅", "WARNING": "⚠️", "DEGRADED": "⚠️", "CRITICAL": "🚨"}
