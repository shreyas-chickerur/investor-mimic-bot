...
class PnLCalculator:
    ...
    def calculate_trade_pnl(
        ...
        if action == "BUY":
            # Add to open lots
            self.open_lots[strategy_id][symbol].append(
                {"shares": shares, "price": price, "date": datetime.now().isoformat()}
            )
    ...
    def _calculate_sell_pnl(
        self, strategy_id: int, symbol: str, shares_to_sell: float, sell_price: float, costs: float
    ) -> tuple[float, str]:
        """
        ...
        """
        if shares_to_sell <= 0:
            logger.warning(f"SELL with zero/negative shares for {symbol}: {shares_to_sell}")
            return 0.0, "SELL: Invalid share count"
    ...
    def get_strategy_pnl_summary(self, strategy_id: int, current_prices: dict[str, float]) -> dict:
        """
        ...
        for symbol in self.open_lots[strategy_id]:
            ...
            if current_price > 0:
                ...
                unrealized_pnl += upnl
                positions.append({"symbol": symbol, **details, "unrealized_pnl": upnl})
