...
class PortfolioRiskManager:
    ...
    def __init__(
    ...
    ):  # Max 40% in correlated positions
        """
        ...
        logger.info(
            f"Portfolio Risk Manager: base_heat={max_portfolio_heat*100:.0f}%, "
    ...
    def set_regime(self, regime: str):
        ...
        if regime in self.regime_heat_limits:
            ...
            if old_limit != self.max_portfolio_heat:
                logger.info(
                    f"Regime changed to {regime}: heat limit adjusted "
    ...
    def check_daily_loss_limit(self, current_value: float) -> bool:
        ...
        if daily_loss < -self.max_daily_loss_pct:
            ...
            logger.error(
                f"CIRCUIT BREAKER: Daily loss {daily_loss*100:.2f}% exceeds limit "
    ...
    def check_portfolio_heat(self, total_exposure: float, portfolio_value: float) -> bool:
        ...
        if heat > self.max_portfolio_heat:
            logger.warning(
                f"Portfolio heat {heat*100:.1f}% exceeds limit "
    ...
    def can_add_position(
        self, position_value: float, current_exposure: float, portfolio_value: float
    ) -> bool:
        """
