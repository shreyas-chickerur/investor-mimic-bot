...
class CorrelationFilter:
    ...
    def __init__(
        ...
        self.price_history: dict[str, Any] = {}  # {symbol: [prices]}

        logger.info(
            f"Correlation Filter: window={correlation_window}d, "
    ...
    def should_filter_signal(
        self, signal_symbol: str, existing_positions: dict[str, float], regime: str = "NORMAL"
    ) -> tuple:
        """
        ...
        for pos_symbol in existing_positions.keys():
            ...
            if use_short_window:
                long_corr, short_corr = self.calculate_correlation_multi_window(
                    signal_symbol, pos_symbol
                )
    ...
    def filter_signals_with_sizing(
        self, signals: list[dict], existing_positions: dict[str, float], market_data: pd.DataFrame
    ) -> list[dict]:
        """
        ...
        for signal in signals:
            ...
            if size_mult > 0:
                ...
                logger.info(
                    f"Accepted {symbol}: size={size_mult*100:.0f}%, "
    ...
    def filter_signals(
        ...
        for signal in signals:
            ...
            symbol = signal.get("symbol")
            should_filter, reason, _ = self.should_filter_signal(symbol, existing_positions)

            if not should_filter:
                filtered_signals.append(signal)
                logger.info(f"Accepted {symbol}: passed correlation filter")
            else:
                logger.info(f"Filtered {symbol}: {reason}")
