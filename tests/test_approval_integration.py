...
class TestApprovalURLRouting:
    ...
    def sample_request(self, manager):
        ...
        trades = [
            {"symbol": "AAPL", "quantity": 10.0, "estimated_price": 185.50, "estimated_value": 1855.0, "allocation_pct": 5.0},
            {"symbol": "GOOGL", "quantity": 5.0, "estimated_price": 296.72, "estimated_value": 1483.6, "allocation_pct": 4.0},
            {"symbol": "MSFT", "quantity": 8.0, "estimated_price": 378.90, "estimated_value": 3031.2, "allocation_pct": 8.0}
        ]
        
        request = manager.create_approval_request(
            trades=trades,
            total_investment=6369.8,
...
class TestBulkApprovalWorkflow:
    ...
    def sample_request(self, manager):
        """Create a sample approval request."""
        trades = [
            {"symbol": "NVDA", "quantity": 12.5, "estimated_price": 495.22, "estimated_value": 6190.25, "allocation_pct": 8.2},
            {"symbol": "META", "quantity": 18.3, "estimated_price": 475.80, "estimated_value": 8707.14, "allocation_pct": 7.5},
            {"symbol": "TSLA", "quantity": 22.0, "estimated_price": 248.50, "estimated_value": 5467.00, "allocation_pct": 6.8},
            {"symbol": "GOOGL", "quantity": 15.2, "estimated_price": 296.72, "estimated_value": 4510.14, "allocation_pct": 5.9},
            {"symbol": "AMZN", "quantity": 8.5, "estimated_price": 178.20, "estimated_value": 1514.70, "allocation_pct": 2.1}
        ]
        
        request = manager.create_approval_request(
            trades=trades,
            total_investment=26389.23,
    ...
    def test_bulk_approval_all_approved(self, client, sample_request, manager):
        ...
        response = client.post(
            f"/api/v1/approve/{sample_request.request_id}/submit",
            data=form_data
        )
...
def run_manual_tests():
    ...
    response = client.post(
        f"/api/v1/approve/{request2.request_id}/submit",
        data=form_data
    )
    assert response.status_code == 200, f"Expected 200, got {response.status_code}"
    assert "Decisions Submitted Successfully" in response.text
