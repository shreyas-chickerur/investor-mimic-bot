...
class TestStopLossManager:
    ...
    def test_stop_triggers_at_3x_atr(self):
        ...
        assert self.manager.check_stop_loss("AAPL", expected_stop) is True

        # Should trigger below stop price
        assert self.manager.check_stop_loss("AAPL", expected_stop - 0.01) is True
    ...
    def test_stop_doesnt_trigger_above_level(self):
        ...
        assert self.manager.check_stop_loss("AAPL", stop_price + 0.01) is False

        # Should NOT trigger at entry price
        assert self.manager.check_stop_loss("AAPL", entry_price) is False

        # Should NOT trigger above entry
        assert self.manager.check_stop_loss("AAPL", entry_price + 10) is False
    ...
    def test_multiple_positions_independent(self):
        ...
        assert self.manager.check_stop_loss("AAPL", 93.0) is True
        assert self.manager.check_stop_loss("MSFT", 190.0) is False
        assert self.manager.check_stop_loss("GOOGL", 145.0) is False
    ...
    def test_stop_removal(self):
        ...
        assert self.manager.check_stop_loss("AAPL", 50.0) is False

    # Unit Test 6: Zero or negative ATR handling
    def test_invalid_atr_handling(self):
        """Zero/negative/None ATR must use a 7% fallback stop, never leave unprotected."""
        # Zero ATR — fallback kicks in
        self.manager.set_stop_loss("AAPL", 100.0, 0.0)
        stop = self.manager.get_stop_price("AAPL")
        assert stop > 0.0, "Position must have a stop even with ATR=0"
        assert 50.0 <= stop < 100.0, f"Fallback stop {stop} outside expected range"

        # Negative ATR — treated same as zero
        self.manager.set_stop_loss("MSFT", 100.0, -1.0)
        stop = self.manager.get_stop_price("MSFT")
        assert stop > 0.0, "Position must have a stop even with ATR<0"

        # None ATR — fallback kicks in
        self.manager.set_stop_loss("GOOGL", 100.0, None)
        stop = self.manager.get_stop_price("GOOGL")
        assert stop > 0.0, "Position must have a stop even with ATR=None"
    ...
    def test_exact_stop_price_triggers(self):
        ...
        assert self.manager.check_stop_loss("AAPL", stop_price) is True
    ...
    def test_small_atr_values(self):
        ...
        assert self.manager.check_stop_loss("AAPL", 99.69) is True
        assert self.manager.check_stop_loss("AAPL", 99.71) is False

    # Unit Test 10: Very large ATR values
    def test_large_atr_values(self):
        """Very large ATR: stop is floored at 50% of entry price."""
        entry_price = 100.0
        atr = 20.0  # 3 × 20 = 60 below entry → $40, but floor is 50% = $50

        self.manager.set_stop_loss("AAPL", entry_price, atr)
        stop_price = self.manager.get_stop_price("AAPL")

        assert stop_price == 50.0, f"Expected floor at $50 (50% of entry), got {stop_price}"
        assert self.manager.check_stop_loss("AAPL", 49.0) is True
        assert self.manager.check_stop_loss("AAPL", 51.0) is False
...
class TestStopLossIntegration:
    ...
    def test_realistic_scenario_aapl(self):
        ...
        assert manager.check_stop_loss("AAPL", 175.0) is False
        assert manager.check_stop_loss("AAPL", 172.0) is False

        # Catastrophic drop should trigger
        assert manager.check_stop_loss("AAPL", 170.0) is True
    ...
    def test_realistic_scenario_volatile_stock(self):
        ...
        assert manager.check_stop_loss("TSLA", 45.0) is False
        assert manager.check_stop_loss("TSLA", 40.0) is False

        # Major drop should trigger
        assert manager.check_stop_loss("TSLA", 37.0) is True
    ...
    class TestStopLossProperties:
        ...
        def test_large_drops_always_trigger(self, entry_price, atr, price_drop_pct):
            ...
            if current_price > 0:  # Ensure price is positive
                assert manager.check_stop_loss("TEST", current_price) is True
