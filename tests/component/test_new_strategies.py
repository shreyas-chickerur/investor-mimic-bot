...
def _make_symbol_data(n_days=100, symbol="TEST", base_price=100.0,
                      base_volume=1_000_000, seed=42):
...
class TestEarningsDriftStrategy:
    ...
    def test_sell_after_drift_window(self):
        ...
        assert 'expired' in sell_signals[0]['reasoning'].lower() or 'drift' in sell_signals[0]['reasoning'].lower()
...
def _make_multi_symbol_data(n_symbols=10, n_days=100, seed=42):
    ...
    for i in range(n_symbols):
        sym = f"SYM{i:02d}"
        df = _make_symbol_data(n_days=n_days, symbol=sym, seed=seed + i,
                               base_price=50 + rng.rand() * 200)
        frames.append(df)
...
class TestStrategyIntegration:
    ...
    def test_both_strategies_produce_valid_signals(self, market_data):
        ...
        for StratClass in [EarningsDriftStrategy, FactorMomentumStrategy]:
            ...
            for sig in signals:
                ...
                assert required_keys.issubset(sig.keys()), f"{StratClass.__name__} missing keys: {required_keys - sig.keys()}"
                assert sig['shares'] > 0
                assert sig['price'] > 0
    ...
    def test_strategies_are_independent(self, market_data):
        ...
        ed_signals = ed.generate_signals(recent)
        ...
        ed_signals2 = ed.generate_signals(recent)
