...
# ---------------------------------------------------------------------------
# Optional password gate — set DASHBOARD_PASSWORD env var to enable.
# When the variable is absent the dashboard runs without auth (safe for local
# development; do not expose publicly without setting a password).
# ---------------------------------------------------------------------------
...
def load_strategy_performance():
    ...
    if not strategy_perf.empty:
        strategy_perf["win_rate"] = (
            strategy_perf["winning_trades"] / strategy_perf["total_trades"] * 100
        ).round(2)
...
with st.sidebar:
    ...
    date_range = st.selectbox(
        "Time Range", ["1 Day", "1 Week", "1 Month", "3 Months", "1 Year", "All Time"]
    )
...
try:
    ...
    with col3:
        if not positions_df.empty:
            ...
            st.metric(
                ...
                if not account.empty and portfolio_value > 0
                else "0%",
            )
        else:
    ...
    if not portfolio_history.empty:
        ...
        fig.update_layout(
            xaxis_title="Date", yaxis_title="Portfolio Value ($)", hovermode="x unified", height=400
        )
    ...
    with col2:
        ...
        if not trades_df.empty:
            ...
            display_trades["executed_at"] = pd.to_datetime(
                display_trades["executed_at"]
