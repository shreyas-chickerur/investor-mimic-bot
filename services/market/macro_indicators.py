...
class MacroIndicatorTracker:
    ...
    def detect_economic_cycle(
        ...
        if (indicators.gdp_growth > 2.0 and 
            indicators.pmi > 52 and 
            indicators.unemployment_rate < 4.5):
            return EconomicCycle.EXPANSION
        
        # Peak: Strong economy but showing signs of overheating
        if (indicators.gdp_growth > 3.0 and
            indicators.inflation_rate > 4.0 and
            indicators.unemployment_rate < 4.0):
            return EconomicCycle.PEAK
        
        # Contraction: Slowing economy, rising unemployment
        if (indicators.gdp_growth < 1.0 or
            indicators.pmi < 50 or
            indicators.unemployment_rate > 5.0 or
            indicators.yield_curve_inverted):
            return EconomicCycle.CONTRACTION
        
        # Trough: Economy bottoming, early recovery signs
        if (indicators.gdp_growth < 0 and
            indicators.pmi > 48 and  # PMI improving but still weak
