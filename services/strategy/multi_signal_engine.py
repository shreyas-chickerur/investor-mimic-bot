...
from services.news.sentiment_analyzer import SimpleSentimentAnalyzer, NewsSignalGenerator
...
class MultiSignalEngine:
    ...
    def compute_multi_signal_scores(
        ...
        holdings = self.conviction_engine.holdings_as_of(
            as_of=date.today(),
            lookback_days=365
        )
        ...
        top_symbols = sorted(
            conviction_weights.items(),
            key=lambda x: x[1],
            reverse=True
        )[:50]  # Top 50 by conviction
    ...
    def get_detailed_analysis(
        ...
        for symbol in symbols:
            ...
            results.append(MultiSignalResult(
                symbol=symbol,
    ...
    def _generate_explanation(
        ...
        if conv_score > 0:
            parts.append(
                f"Top investors hold this stock (conviction score: {conv_score:.3f})"
            )
        
        if sent_score > 0.3:
            parts.append(
                f"Very positive news sentiment ({article_count} recent articles)"
            )
        elif sent_score > 0:
            parts.append(
                f"Positive news sentiment ({article_count} recent articles)"
            )
        elif sent_score < -0.3:
            parts.append(
                f"Negative news sentiment ({article_count} recent articles)"
            )
