...
class ExtendedDataFetcher:
    ...
    def __init__(self, api_key: str | None = None, years: int = 15, premium: bool = True):
        """
        Initialize fetcher
    ...
    def fetch_stock_data(self, symbol: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"  {symbol}: {len(df)} days ({df['date'].min().date()} to {df['date'].max().date()})"
            )
    ...
    def _fetch_parallel(self) -> pd.DataFrame:
        ...
        logger.info(
            f"Fetching {len(STOCK_SYMBOLS)} stocks in parallel ({self.max_workers} workers)..."
        )
        logger.info(
            f"Estimated time: {len(STOCK_SYMBOLS) / self.max_workers / 75 * 60:.1f} minutes"
        )
    ...
    def _fetch_sequential(self) -> pd.DataFrame:
        ...
        delay_seconds = 12
        logger.info(
            f"Fetching {len(STOCK_SYMBOLS)} stocks sequentially ({delay_seconds}s delay)..."
        )
