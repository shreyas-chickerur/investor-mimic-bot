...
def verify_terminal_states():
    ...
    cursor.execute("SELECT COUNT(*) FROM signals WHERE DATE(generated_at) = ?", (today,))
    total_signals = cursor.fetchone()[0]
    
    cursor.execute("""
        SELECT COUNT(*) FROM signals 
        WHERE DATE(generated_at) = ? AND terminal_state IS NOT NULL
    """, (today,))
    with_terminal = cursor.fetchone()[0]
    
    cursor.execute("""
        SELECT COUNT(*) FROM signals 
        WHERE DATE(generated_at) = ? AND terminal_state IS NULL
    """, (today,))
