...
def get_signal_reasons(db, symbols: List[str]) -> Dict[Tuple[str, str], dict]:
    ...
    for r in rows:
        action = 'BUY' if (r.get('signal_type') or '').upper() in ('BUY', 'LONG', 'ENTRY') else 'SELL'
        key = (r['symbol'], action)
...
def _infer_sell_reason(strategy: str, signal_reasoning: str,
                        pnl: Optional[float], pnl_pct: Optional[float],
                        days_held: Optional[int]) -> str:
    ...
    if 'rsi' in r and ('>' in r or 'recovery' in r or 'reversion' in r or 'complete' in r):
        return f"RSI recovered — reversion complete ({pct_str})" if pct_str else "RSI recovered — reversion complete"
    if 'negative' in r and ('surprise' in r or 'sentiment' in r):
        return f"Negative signal — exited early ({pct_str})" if pct_str else "Negative signal — exited early"
...
def build_today_trades(trades: List[dict],
...
"""
    ...
    for t in trades:
        ...
        if is_buy:
            ...
            pnl_cell = (f'<span style="color:{TEXT_DIM};font-family:{FONT};'
            ...
            pnl_cell = (f'<span style="font-family:{MONO};font-size:14px;'
...
# ── Main entry ────────────────────────────────────────────────────────────────
def generate_email_body(db_path: str = 'trading.db',
...
<body style="margin:0;padding:0;background:{BG};">
  <div style="max-width:720px;margin:0 auto;background:{BG};">
    {body}
  </div>
</body></html>"""
...
def main() -> int:
    ...
    parser.add_argument('--include-visuals', action='store_true',
                        help='Compatibility flag — visuals always on now.')
