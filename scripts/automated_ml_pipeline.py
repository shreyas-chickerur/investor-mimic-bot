...
class AutomatedMLPipeline:
    ...
    def weekly_model_training(self):
        ...
        try:
            notify(
                "Weekly model training started", channel=NotificationChannel.SLACK, priority=NotificationPriority.MEDIUM
            )
...
Status: {'✅ Performing well' if metrics.get('accuracy', 0) > 0.55 else '⚠️ Needs attention'}
            ...
            notify(report, channel=NotificationChannel.SLACK, priority=NotificationPriority.LOW)
    ...
    def run(self):
        ...
        notify("🚀 Automated ML Pipeline Started", channel=NotificationChannel.SLACK, priority=NotificationPriority.LOW)
        ...
        while True:
            try:
                ...
                notify(
                    f"⚠️ Pipeline Error: {e}", channel=NotificationChannel.SLACK, priority=NotificationPriority.CRITICAL
                )
