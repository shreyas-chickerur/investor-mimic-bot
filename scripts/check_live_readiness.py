...
def evaluate_readiness(db_path: str = 'trading.db') -> Dict:
    ...
    checks: List[Dict] = [
        _check('Closed trades',         closed,
               f'>= {REQUIRED_TRADES}',
               closed >= REQUIRED_TRADES,                 fmt='{}'),
        _check('Win rate',              (win_rate or 0) * 100 if win_rate is not None else None,
               f'>= {int(MIN_WIN_RATE*100)}%',
               win_rate is not None and win_rate >= MIN_WIN_RATE, fmt='{:.1f}%'),
        _check('Max drawdown',          max_dd,
               f'<= {MAX_ALLOWED_DRAWDOWN:.1f}%',
               max_dd is not None and max_dd <= MAX_ALLOWED_DRAWDOWN, fmt='{:.2f}%'),
        _check('Profit factor',         profit_fact,
               f'>= {MIN_PROFIT_FACTOR}',
               profit_fact is not None and profit_fact >= MIN_PROFIT_FACTOR, fmt='{:.2f}'),
        _check('Clean reconciliation',  clean_streak,
               f'>= {REQUIRED_CLEAN_RECON} days',
               clean_streak >= REQUIRED_CLEAN_RECON, fmt='{} days'),
    ]
    ...
    return {
        ...
        'summary':          (
            'READY to disable paper trading.' if ready else
            f'NOT READY — {passed}/{total} checks pass.'
        ),
    }
