...
engine = MultiSignalEngine(
    signal_weights=SignalWeights(
        conviction_13f=0.70,  # 70% weight on 13F
        news_sentiment=0.30   # 30% weight on news
    ),
...
try:
    ...
    if fetch_news and len(sorted_scores) >= 3:
        ...
        for symbol in top_3_symbols:
            ...
            print(f"  Conviction Score: {explanation['conviction_score']:.4f} (weight: {explanation['conviction_weight']:.0%})")
            print(f"  Sentiment Score:  {explanation['sentiment_score']:.4f} (weight: {explanation['sentiment_weight']:.0%})")
