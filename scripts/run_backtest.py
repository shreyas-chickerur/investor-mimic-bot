...
def main():
    ...
    # Strategy classes to test (portfolio backtest uses the four with the most history)
    ...
    if args.per_strategy:
        _run_per_strategy_backtest(df, args, ALL_STRATEGY_CLASSES)
        return
...
def _run_per_strategy_backtest(
    df: pd.DataFrame, args: "argparse.Namespace", strategy_classes: list
) -> None:
    ...
    for r in sorted(
        ...
        dd = f"{r['max_drawdown']:.1%}" if r.get("max_drawdown") else "—"
