...
def main():
    ...
    try:
        ...
        filtered_signals = {
            s: raw_scores[s] for s in normalized_symbols
            if s in volatilities and s in raw_scores
        }
        ...
        adjusted_sizes = risk_manager.adjust_for_market_regime(
            optimal_sizes,
            regime
        )
        ...
        is_valid, violations = risk_manager.validate_portfolio_risk(
            adjusted_sizes,
            sector_map
        )
        ...
        positions_for_var = {
            s: {
                'value': float(size),
                'volatility': volatilities.get(s, 0.20)
            }
        ...
        for trade in trades:
            print(f"  {trade['symbol']:<10} {trade['quantity']:>8.2f} shares @ ${trade['estimated_price']:>8.2f} = ${trade['estimated_value']:>10,.2f} ({trade['allocation_pct']:>5.1f}%) [Vol: {trade['volatility']:.1f}%]")
        ...
        performance_tracker.record_snapshot(
            ...
            cash=buying_power - positions_value,
