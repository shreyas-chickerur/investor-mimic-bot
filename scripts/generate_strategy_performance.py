...
def rank_strategies(performance):
    ...
    if not active_strategies:
        return {"by_pnl": [], "by_win_rate": [], "by_sharpe": [], "by_profit_factor": []}

    rankings = {
        "by_pnl": sorted(
            active_strategies, key=lambda s: performance[s]["total_pnl"], reverse=True
        ),
        "by_win_rate": sorted(
            active_strategies, key=lambda s: performance[s]["win_rate"], reverse=True
        ),
        "by_sharpe": sorted(
            active_strategies, key=lambda s: performance[s]["sharpe_ratio"], reverse=True
        ),
        "by_profit_factor": sorted(
            active_strategies, key=lambda s: performance[s]["profit_factor"], reverse=True
        ),
    }
...
def generate_strategy_summary(performance, rankings, days=30):
    ...
    for strategy in sorted(performance.keys()):
        ...
        summary += (
            f"  Trades: {perf['total_trades']} total ({open_pos} open positions, {closed} closed)\n"
        )
        if closed == 0 and open_pos > 0:
            summary += "  P&L: $0.00 (all positions still open — unrealized P&L pending close)\n"
        else:
