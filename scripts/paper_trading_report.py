...
def run_report(days: int = 30) -> None:
    ...
    # ── Trade P&L detail ──────────────────────────────────────────────────────
    trades = conn.execute(
        "SELECT * FROM trade_pnl_detail WHERE exit_date >= ? ORDER BY exit_date",
        (since,)
    ).fetchall()
    ...
    if snaps:
        ...
        min_val = min(s['portfolio_value'] for s in snaps)
        ...
        implied_cagr = ((last_val / first_val) ** (252 / max(trading_days, 1)) - 1) if first_val > 0 else 0.0

        print(f"  Period          : {snaps[0]['snapshot_date']} → {snaps[-1]['snapshot_date']}  ({trading_days} trading days)")
        ...
        for s in snaps[-10:]:  # last 10 rows
            ...
            print(f"  {s['snapshot_date']:<12} ${s['portfolio_value']:>9,.2f} {dpnl:>10} {dd:>10} {regime:<20} {vix:>6}")
    else:
    ...
    if trades:
        ...
        print(f"  Win/Loss ratio  : {abs(avg_w / avg_l):.2f}x" if avg_l else "  Win/Loss ratio  : ∞")
    ...
    if funnel:
        ...
        for row in funnel:
            ...
            print(f"  {row['strategy_name']:<28} {row['raw']:>5} {row['after_regime']:>7} {row['after_corr']:>6} {row['executed']:>6} {fill_pct:>6.1f}%")
    else:
