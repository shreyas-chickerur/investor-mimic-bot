...
from services.execution.alpaca_executor import AlpacaExecutionConfig, AlpacaExecutionService
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-require-market-open",
        action="store_false",
        dest="require_market_open",
        help="Disable market-hours gating (not recommended)",
    )
    ...
    if str(args.confirm).strip() != "EXECUTE_PAPER" and paper:
        raise RuntimeError("Missing confirmation token: --confirm EXECUTE_PAPER")

    if str(args.confirm).strip() != "EXECUTE_LIVE" and not paper:
        raise RuntimeError("Missing confirmation token: --confirm EXECUTE_LIVE")

    exec_cfg = AlpacaExecutionConfig(require_market_open=bool(args.require_market_open))
    executor = AlpacaExecutionService(trading, cfg=exec_cfg)

    results = executor.place_limit_buy_orders(
        plan,
        max_orders=int(args.max_orders),
        confirm_market_open=bool(args.require_market_open),
    )
