...
class OptimizedBacktester:
    ...
    def create_comparison_visualization(
        ...
        axes[0, 0].plot(baseline_df.index.values, baseline_df['value'].values, 
                       linewidth=2, label='Baseline', alpha=0.7)
        axes[0, 0].plot(optimized_df.index.values, optimized_df['value'].values, 
                       linewidth=2, label='Optimized', alpha=0.7)
        ...
        axes[1, 0].plot(baseline_cum.index.values, baseline_cum.values, 
                       linewidth=2, label='Baseline', alpha=0.7)
        axes[1, 0].plot(optimized_cum.index.values, optimized_cum.values, 
                       linewidth=2, label='Optimized', alpha=0.7)
        ...
        improvements = {
            'Total Return': ((optimized_metrics['total_return'] - baseline_metrics['total_return']) / 
                           abs(baseline_metrics['total_return'])) * 100,
            'Sharpe Ratio': ((optimized_metrics['sharpe_ratio'] - baseline_metrics['sharpe_ratio']) / 
                           baseline_metrics['sharpe_ratio']) * 100,
            'Trades': ((baseline_metrics['total_trades'] - optimized_metrics['total_trades']) / 
                      baseline_metrics['total_trades']) * 100
        }
...
def main():
    ...
    latest_baseline = sorted(baseline_results_dir.glob('*/metrics.json'))[-1] if list(baseline_results_dir.glob('*/metrics.json')) else None
