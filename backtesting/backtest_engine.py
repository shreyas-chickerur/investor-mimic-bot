    ...
    def __post_init__(self):
        if self.exclude_periods is None:
            ...
            self.exclude_periods = [
                (datetime(2020, 3, 1), datetime(2020, 6, 30))
            ]
...
class BacktestEngine:
    ...
    def calculate_metrics(self, spy_returns: pd.Series) -> Dict:
        ...
        profit_factor = (sum([self._trade_pnl(t) for t in winning_trades]) / 
                        sum([abs(self._trade_pnl(t)) for t in losing_trades])) if losing_trades else 0
